...
const QuestionForm = ({
  ...
  const first =
    index !== -1
      ? currentPart?.SubContent?.slice(0, index + 1)
      : currentPart?.SubContent;
  const rest =
    index !== -1 ? currentPart?.SubContent?.slice(index + 1).trim() : "";
  ...
  const content = currentPart?.SubContent?.slice(
    commaIndex + 1,
    lastPeriodIndex + 1
  ).trim();
          ...
          return (
            <Form.Item
              ...
              label={
                <Text className="text-base">
                  {(() => {
                    const cleanedContent = question.Content.replace(
                      /^\d+\.\s*/,
                      ""
                    )
                      .replace(/\(\d+(\.\d+)?\s*points?\)/i, "")
            ...
            >
              <Input.TextArea
                ...
                onChange={(e) =>
                  handleTextAreaChange(fieldName, e.target.value, maxWords)
                }
