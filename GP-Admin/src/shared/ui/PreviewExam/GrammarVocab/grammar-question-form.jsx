...
const CustomMatchingQuestionEquals = ({
  leftItems,
  rightItems,
  userAnswer = [],
  setUserAnswer,
  ...
  return (
    <div className="mx-auto rounded-xl bg-white">
      <Row justify="center">
        <Col span={18}>
          <div className="w-full py-4 text-lg font-medium">
...
const CustomMatchingQuestionPlus = ({
  leftItems,
  rightItems,
  userAnswer = [],
  setUserAnswer,
...
const formatDialogueContent = (content) => {
  ...
  if (
    content.includes(":") &&
    (content.includes("____") || content.includes("__"))
  ) {
...
const QuestionForm = ({
  ...
  const questionContent =
    currentPart.Content ||
    (currentPart.AnswerContent.title ? currentPart.AnswerContent.title : null);
  ...
  return (
    <Form layout="vertical">
      {currentPart.Type === "matching" && (
        <div className="mb-6">
          <Paragraph className="mb-0 text-xl font-semibold">
            {currentPart.Content}
          </Paragraph>
        </div>
      )}
      {currentPart.Type === "multiple-choice" && (
        <div className="mb-6">{formatDialogueContent(questionContent)}</div>
      )}
      <Form.Item
