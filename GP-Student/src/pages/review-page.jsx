    ...
    <Layout className="min-h-screen bg-white">
      ...
      <Content className="mx-auto w-full max-w-7xl p-6">
        ...
        <div className="mt-8">
          {isSpeakingTab ? (
          ...
          ) : currentSkillData?.questions?.length > 0 ? (
            <div className="flex flex-col gap-8">
              {currentSkillData.questions.map((q, idx) => {
                ...
                const isGroupQuestion =
                  qType === 'listening-questions-group' || (rawContent && String(rawContent).includes('listContent'))
                const isFullyCorrect = checkIsFullyCorrect(q)

                return (
                  <div key={q.id} className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
                    <div className="mb-4">
                      <Title level={4} className="!mb-4">{`Question ${idx + 1}:`}</Title>
                      <QuestionHeaderDisplay question={q} />
                    </div>
                    {activeTab === 'writing' ? (
                      <SubjectiveAnswerView question={q} />
                    ) : (
                      !isGroupQuestion && <AnswerComparison question={q} />
                    )}

                    {activeTab !== 'writing' && !isGroupQuestion && isFullyCorrect && (
            ...
            <div className="flex flex-col items-center justify-center p-10 text-gray-500">
              <WarningOutlined className="mb-2 text-3xl" />
